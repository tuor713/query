<script>
    ...
    function deleteFolder(folderId, e) {
        e.stopPropagation();
        // Move all items in folder to top level before deleting
        const itemsInFolder = items.filter((q) => q.folderId === folderId);
        itemsInFolder.forEach((q) => {
            onItemMoved(q.name, null);
        });
    ...
    function handleDrop(e, targetFolderId) {
        e.preventDefault();
        if (draggedItem) {
            onItemMoved(draggedItem.name, targetFolderId);
        }
        draggedItem = null;
        dragOverTarget = null;
    }
            ...
            <li class="folder-item">
                ...
                {#if expandedFolders.has(folder.id)}
                    <ul class="folder-contents">
                        {#each getItemsInFolder(folder.id) as item}
                            <li
                                class="item-row"
                                draggable="true"
                            ...
                            >
                                ...
                                >
                                    <span
                                        class={item.name === currentItemName
                                            ? "label selected"
                                            : "label"}
                                    >
                                        {item.name}
                        ...
                        {/each}
                        {#if getItemsInFolder(folder.id).length === 0}
                            <li class="empty-folder">Drop items here</li>
                        {/if}
                    </ul>
                {/if}
            </li>
...
<style>
    ...
    .sidebar ul li .item-button,
    .sidebar .item-row .item-button {
        background: none;
        border: none;
    ...
    .sidebar ul li .item-button:hover,
    .sidebar .item-row .item-button:hover {
